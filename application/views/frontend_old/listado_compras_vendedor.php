      ...
      <div id="content">
        <div class="wrap-featured-products">
            <div class="wrap-title-black">
              <h1 class="nice-title">Listado de Compras de Vendedor</h1>
            </div>
            <div class="contenido">
            <?php
                if(count($ordenes)>0)
                {
            ?>
            <h2>Listado de Compras de <?php echo $vendedor['nombre']; ?></h2>
			...
			echo '<td align="center" class="datoReserva">'.$codigo_orden.'</td>';       
                        ...
                        echo '<td align="center" class="datoReserva">S/ '.$total.'</td>';
                        ...
                        echo '<a class="btnVerde" href="vendedores/detalleCompra/'.$id_vendedor.'/'.$id_orden.'">DETALLE</a><br />';
                        echo '</td>';
                        echo '</tr>';
                        $i++;
                    }                
            ...
            //for($w=1; $w<=$num_paginas; $w++)
            {
               if($w==$pagina)
                {
                    echo '<li><a href="vendedores/listaCompras/'.$id_vendedor.'/'.$w.'" class="actual">'.$w.'</a></li>';
                }
                else
                {
                    echo '<li><a href="vendedores/listaCompras/'.$id_vendedor.'/'.$w.'" >'.$w.'</a></li>';
                }
            }
            ?>
            </ul>
            ...
            <?php
                ...
                {
                    echo '<h2>Listado de Compras de '.$vendedor['nombre'].'</h2>';
                    echo '<div class="msgAlerta">En este momento este vendedor no tiene compras registradas</div>';
                }
            ?>
            </div><!-- contenido -->
